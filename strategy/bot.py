...
class bot(object):
    def __init__(self)-> None:
        self.userid_exid_symbol_tf_strategy = dict()
    ...
    def init_system_strategy(self):
        t_markets_list = db.t_markets.query.group_by(db.t_markets.f_ex_id, db.t_markets.f_symbol).all()
        for t_markets in t_markets_list:
            for tf in util.Minutes_TimeFrame.keys():
                    self.userid_exid_symbol_tf_strategy[self.userid_system] = {
                        t_markets.f_ex_id : {
                            t_markets.f_symbol : {
                                tf : None
                            }
        ...
        for t_markets in t_markets_list:
            for tf in util.Minutes_TimeFrame.keys():
                    ...
                    self.userid_exid_symbol_tf_strategy[self.userid_system][t_markets.f_ex_id][t_markets.f_symbol][tf] = a

    def load_user_strategy(self):
        t_list = db.t_user_trades_config.query.all()
        for t_user_trades_config in t_list:
            self.userid_exid_symbol_tf_strategy[t_user_trades_config.f_userid] = {
                t_user_trades_config.f_ex_id : {
                    t_user_trades_config.f_symbol : {
                        1 : None
                    }
                }
            }
        for t_user_trades_config in t_list:
            user_strategy = strategy.load_strategy(t_user_trades_config.f_strategy)
            if user_strategy is None:
                continue
            a = analyze(self.userid_system, t_user_trades_config.f_ex_id, t_user_trades_config.f_symbol, user_strategy._timeframe, user_strategy)
            self.userid_exid_symbol_tf_strategy[t_user_trades_config.f_userid][t_user_trades_config.f_ex_id][t_user_trades_config.f_symbol][user_strategy._timeframe] = a
